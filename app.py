...
if st.button("Fill Form"):
    if user_input.strip():
        with st.spinner("Preparing your file..."):
            query = "For the given prompt:"+user_input+",analyze it very well and understand it and then answer the questions of the following dictionary:"+json.dumps(form_data.get(selected_form))+", and fill this dictionary with the correct answers:"+json.dumps(formAnswer.get(selected_form))+"and the answer of {{checks}} is either 'true' or 'false' only only as a string only, if you do not know if it is 'true' or 'false' then ignore it and don't include it in the answers json and don't mention it at all or delete it from the answers dictionary, for example: instead of writing this: ( {{Check_87}}: false, {{Check_88}}: N/A, {{received_treatment}}: N/A, {{Check_89}}: true ) just write this: ({{Check_87}}: false, {{Check_89}}: true ) and ignore nulls and N/A, just skip it and delete it from the answers, and analyze carefully before answering. Make sure to fill ALL the fields of the given sample and NEVER put a null value, delete it and never mention it if and only if when the answer can not be determined or concluded or interpreted or analyzed from the prompt. Give me the result directly in json format with nothing written before or after and DO NOT SKIP AN ENTRY IN THE ANSWERS file or the dictionary file especially {{check_33}}, and the answers must not exceed 40 words, no 'answer' having more than 40 words is acceptable. Try to elaborate your answers within those 40 words even if there is no enough data try to analyze them, do not write very brief answers. Dont include the placeholder and its answer when it is null or N/A, delete it and dont write it in your response. Don't include any key value in your response if the value is 'answer', 'null','N/A','NA'. Give me the result directly in json format as a string with nothing written before or after!"

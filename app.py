...
if st.button("Fill Form"):
    if user_input.strip():
        with st.spinner("Preparing your file..."):
            query = "For the given prompt:"+user_input+",analyze then answer the questions of the following dictionary:"+json.dumps(form_data.get(selected_form))+", and fill this dictionary with the correct answers:"+json.dumps(formAnswer.get(selected_form))+"and the answer of {{checks}} is either 'true' or 'false' as a string only, if you do not know if it is true or false then put 'false'. Make sure to fill ALL the fields of the given sample and NEVER put a null value, put NA instead, however for {{checks}} like {{check_14}} put 'false' if you do not know or have no answer, and never put 'true' or 'false' for anything except {{checks}}, for example: {{reunification}}:'no' instead of {{reunification}}:'false'. Give me the result directly in json format with nothing written before or after and DO NOT SKIP AN ENTRY IN THE ANSWERS file or the dictionary file especially {{check_33}}, and the answers must not exceed 30 words, no 'answer' having more than 30 words is acceptable. Give me the result directly in json format as a string with nothing written before or after!"
